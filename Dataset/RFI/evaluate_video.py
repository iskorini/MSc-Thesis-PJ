...
def iou_evaluation(detections, threshold):
    ...
    for detection in np.concatenate(np.delete(detections, index_max_detections, axis = 0)):
    ...
    iou_values = np.divide(iou_values, len(detections)-1) # ma che senso ha questa riga?
    bbox = []
    labels = []
    scores = []
